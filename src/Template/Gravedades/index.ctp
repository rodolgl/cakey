...
<div class="gravedades index large-9 medium-8 columns content">
    <h3><?= __('Gravedades') ?></h3>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($gravedades as $gravedad): ?>
            <tr>
                <td><?= h($gravedad->nivel) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $gravedad->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $gravedad->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $gravedad->id], ['confirm' => __('Are you sure you want to delete # {0}?', $gravedad->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
